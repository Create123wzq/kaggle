...
#统计缺失值
# a = full.isnull().sum()
# a[a>0].sort_values(ascending=False).to_csv('Result.csv')
...
# Combining different features is usually a good way, but we have no idea what features should we choose.
# Luckily there are some models that can provide feature selection,
# here I use Lasso, but you are free to choose Ridge, RandomForest or GradientBoostingTree.
lasso = Lasso(alpha=0.001)
lasso.fit(X_scaled, y_log)
FI_lasso = pd.DataFrame({"Feature Importance": lasso.coef_}, index=data_pipe.columns)
# FI_lasso[FI_lasso["Feature Importance"]!=0].sort_values("Feature Importance").plot(kind='barh', figsize=(15, 15))
...
# PCA
# Im my case, doing PCA is very important. It lets me gain a relatively big boost on leaderboard.
# At first I don't believe PCA can help me, but in retrospect, maybe the reason is that the features I built are highly correlated, and it leads to multicollinearity.
# PCA can decorrelate these features.
...
def rmse_cv(model, X, y):
    rmse = np.sort(-cross_val_score(model, X, y, scoring='neg_mean_squared_error', cv=5))
    return rmse
...
class AverageWeight(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        self.models = [clone(x) for x in self.mod]
    ...
    def predict(self, test_x):
        w = list()
        pred = np.array([model.predict(test_x) for model in self.models])
...
print(rmse_cv(stack_model,X_train_add,b).mean()) #overfitting
# submission
